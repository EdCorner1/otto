...
function parseCreatorMeta(tags: Array<{ tag: string }>) {
  ...
  return {
    ...
    backgroundStyle: read('background:') || 'plain',
    nicheTags: tags
...
export async function GET(_request: NextRequest, context: RouteContext) {
  try {
    ...
    const { data, error } = await admin
      .from('creators')
      .select('id, user_id, display_name, bio, avatar_url, booking_url, creator_socials(platform, url), creator_tags(tag), portfolio_items(id, url, type, platform, caption, thumbnail_url, created_at, sort_order)')
      .eq('id', id)
      .single()
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    const backgroundStyle = ['plain', 'grid', 'dots'].includes(String(body.backgroundStyle || '')) ? String(body.backgroundStyle) : 'plain'
    const bookingUrl = body.bookingUrl ? String(body.bookingUrl).trim() : null
    ...
    const managedTagPrefixes = ['handle:', 'main_platform:', 'followers:', 'income:', 'niche:', 'intro_video:', 'background:']
