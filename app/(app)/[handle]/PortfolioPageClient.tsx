...
function portfolioBackgroundStyle(style: PublicCreatorPortfolio['backgroundStyle']) {
  if (style === 'grid') {
    return {
  ...
  if (style === 'dots') {
    return {
      backgroundColor: '#fafaf9',
      backgroundImage: `radial-gradient(circle, rgba(54,53,53,0.11) 1px, transparent 1.2px)`,
...
function BookingSection({ creatorId, bookingUrl }: { creatorId: string; bookingUrl?: string | null }) {
  const [open, setOpen] = useState(false)
  const [submitting, setSubmitting] = useState(false)
  const [done, setDone] = useState(false)
  const [error, setError] = useState('')
  const [fields, setFields] = useState({ brandName: '', brandEmail: '', brandCompany: '', budget: '', timeline: '', message: '' })
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/inquiries', {
        ...
        body: JSON.stringify({ creatorId, ...fields }),
      })
      if (!res.ok) {
  ...
  if (done) {
    return (
      <div className="flex flex-col items-center gap-3 py-2">
        <CheckCircle className="h-8 w-8 text-[#7ea400]" />
        <p className="text-sm font-semibold text-[#1c1c1e]">Inquiry sent!</p>
        <p className="text-sm text-[#6b6b66]">You&apos;ll hear back soon.</p>
      </div>
    )
  }
  ...
  if (open || bookingUrl) {
    if (bookingUrl) {
      return (
        <div className="flex flex-col items-center gap-3">
          ...
          <a href={bookingUrl} target="_blank" rel="noopener noreferrer" className="mt-1 inline-flex items-center justify-center rounded-full bg-[#ccff00] px-6 py-3 text-sm font-semibold text-[#1c1c1e] transition hover:bg-[#d8ff47]">
            Book a call directly
          </a>
          <button onClick={() => setOpen(true)} className="mt-2 text-xs text-[#8b8b84] underline hover:text-[#1c1c1e]">
    ...
    return (
      <form onSubmit={submit} className="flex flex-col gap-3 text-left">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#8b8b84]">Send an inquiry</p>
        <div className="grid gap-3 sm:grid-cols-2">
          <input required placeholder="Brand name" value={fields.brandName} onChange={set('brandName')} className="rounded-xl border border-[#e4e4dd] bg-[#fafaf7] px-4 py-2.5 text-sm text-[#1c1c1e] placeholder-[#b0b0a8]" />
          <input required type="email" placeholder="Work email" value={fields.brandEmail} onChange={set('brandEmail')} className="rounded-xl border border-[#e4e4dd] bg-[#fafaf7] px-4 py-2.5 text-sm text-[#1c1c1e] placeholder-[#b0b0a8]" />
          <input placeholder="Company / product" value={fields.brandCompany} onChange={set('brandCompany')} className="rounded-xl border border-[#e4e4dd] bg-[#fafaf7] px-4 py-2.5 text-sm text-[#1c1c1e] placeholder-[#b0b0a8]" />
          <select value={fields.budget} onChange={set('budget')} className="rounded-xl border border-[#e4e4dd] bg-[#fafaf7] px-4 py-2.5 text-sm text-[#1c1c1e]">
        ...
        <select value={fields.timeline} onChange={set('timeline')} className="rounded-xl border border-[#e4e4dd] bg-[#fafaf7] px-4 py-2.5 text-sm text-[#1c1c1e]">
        ...
        <textarea required rows={3} placeholder="Tell me about the campaign — what are you making, what's the goal, any links?" value={fields.message} onChange={set('message')} className="rounded-xl border border-[#e4e4dd] bg-[#fafaf7] px-4 py-2.5 text-sm text-[#1c1c1e] placeholder-[#b0b0a8] resize-none" />
        {error && <p className="flex items-center gap-1.5 text-sm text-[#d14343]"><AlertCircle className="h-4 w-4" />{error}</p>}
        <button type="submit" disabled={submitting} className="flex items-center justify-center gap-2 rounded-full bg-[#ccff00] px-6 py-3 text-sm font-semibold text-[#1c1c1e] transition hover:bg-[#d8ff47] disabled:opacity-60">
          {submitting ? <><Loader2 className="h-4 w-4 animate-spin" /> Sending…</> : 'Send inquiry'}
        </button>
      </form>
    )
  }
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#8b8b84]">Calendar</p>
      <h3 className="text-2xl leading-tight text-[#1c1c1e]" style={{ fontFamily: 'var(--font-bricolage)', letterSpacing: '-0.04em' }}>Book a call or send an inquiry</h3>
      <p className="mx-auto max-w-md text-sm leading-6 text-[#6b6b66]">Skip the back-and-forth — book directly if the creator has a calendar set up, or fill out a quick inquiry and they&apos;ll get back to you within 24 hours.</p>
      <div className="mt-2 flex gap-3">
        <button onClick={() => setOpen(true)} className="inline-flex items-center justify-center rounded-full bg-[#ccff00] px-6 py-3 text-sm font-semibold text-[#1c1c1e] transition hover:bg-[#d8ff47]">
          Send inquiry
        </button>
        <a href={bookingUrl || 'https://ottougc.com'} className="inline-flex items-center justify-center rounded-full border border-[#e4e4dd] bg-white px-6 py-3 text-sm font-semibold text-[#1c1c1e] transition hover:-translate-y-0.5 hover:border-[#ccff00]">
...
export default function PortfolioPageClient({
  ...
  return (
    <>
      <main className="min-h-screen text-[#363535]" style={{ fontFamily: 'var(--font-open-sans)', ...portfolioBackgroundStyle(portfolio.backgroundStyle) }}>
              ...
              <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_380px] lg:items-center xl:grid-cols-[minmax(0,1fr)_400px]">
                <div className="max-w-4xl">
                  ...
                  <div className="mt-7 flex flex-wrap items-center gap-3">
                    <a
                      href={portfolio.bookingUrl || primaryContactHref}
