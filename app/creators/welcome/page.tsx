    ...
    try {
      if (email) {
        await fetch('/api/waitlist-profile', {
          ...
          body: JSON.stringify({ email, ...data, role: 'creator' }),
        })
      }
    } catch { /* non-fatal */ }
    setDone(true)
      ...
      <main className="mx-auto flex min-h-screen max-w-lg items-center justify-center px-6 pt-28 pb-12">
        <div className="w-full">
          ...
          <p className="mb-8 text-sm text-[#6b6b6b]">
            {email ? `We’ve saved your waitlist spot for ${email}. Answer a few quick questions so we can shape the first creator invites.` : 'Answer a few quick questions so we can shape the first creator invites.'}
          </p>
